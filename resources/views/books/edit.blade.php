...
<div class="px-4 sm:px-6 lg:px-8">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-8 text-center">
            <div class="inline-flex items-center justify-center w-20 h-20 rounded-full mb-4 shadow-lg" style="background: linear-gradient(135deg, #a78bfa 0%, #ec4899 100%);">
                <svg class="w-10 h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                </svg>
            </div>
            <h1 class="text-4xl font-bold text-slate-800 mb-2">Edit Book</h1>
            <p class="text-slate-600">Update the book information</p>
        </div>

        <!-- Form Card -->
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50 overflow-hidden">
            <form action="{{ route('books.update', $book) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="isbn" id="isbn" value="{{ old('isbn', $book->isbn) }}" required maxlength="20"
                        ...
                        @if($book->image_path)
                            <div class="mb-4">
                                <img src="{{ str_starts_with($book->image_path, 'http') ? $book->image_path : asset('storage/' . $book->image_path) }}" 
                                     alt="{{ $book->title }}" 
                                     class="max-w-xs rounded-xl shadow-lg border-2 border-slate-200">
                            </div>
                        @else
                            <div class="mb-4 p-4 bg-slate-50 rounded-xl border-2 border-slate-200">
                                <p class="text-sm text-slate-500">No cover image currently set</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <label for="image" class="block text-sm font-semibold text-slate-700 mb-2">
                            <span class="flex items-center">
                                ...
                                Update Book Cover Image (Upload)
                            </span>
                        </label>
                        ...
                        <p class="mt-1 text-xs text-slate-500">Upload a new book cover image (max 2MB, jpeg, png, jpg, gif, webp)</p>
                    ...
                    <div class="md:col-span-2">
                        <label for="image_url" class="block text-sm font-semibold text-slate-700 mb-2">
                            <span class="flex items-center">
                                ...
                                Or Update Book Cover Image URL
                            </span>
                        </label>
                ...
                <div class="flex items-center justify-end space-x-4 pt-6 border-t border-slate-200">
                    ...
                    <button type="submit" class="px-8 py-3 rounded-xl font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 flex items-center" style="background: linear-gradient(135deg, #a855f7 0%, #ec4899 100%) !important; color: #fef3c7 !important; border: none;">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="color: #fef3c7 !important;">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                        </svg>
                        <span style="color: #fef3c7 !important;">Update Book</span>
                    </button>
                </div>
            </form>
        </div>
